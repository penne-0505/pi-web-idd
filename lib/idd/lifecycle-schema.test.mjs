...
test("emit: missing required attr fails with names listed", () => {
  const r = emitLifecycleLine({
    ...
    ts: TS,
    repo: "flutter",
    attrs: { linear_issue_id: "APP-1" }, // missing worktree_branch, worker
  });
  assert.equal(r.ok, false);
...
test("emit: field order in output matches schema declaration", () => {
  // s3_ready: required=[linear_issue_id, narrative], optional=[touched_paths, ac_coverage]
  const r = emitLifecycleLine({
    event: "s3_ready",
    ts: TS,
    repo: "flutter",
  ...
  assert.equal(r.ok, true);
  // Field order: ts, repo, type, linear_issue_id, narrative, touched_paths, ac_coverage
  assert.deepEqual(
...
test("emit: attrs wins over attrsJson on key collision", () => {
  ...
  // touched_paths from attrsJson is preserved (no collision)
...
test("emit: all 13 events produce valid output when given required attrs", () => {
  // Ensure schema and code stay in lockstep — every declared event must be emittable.
  const minimalAttrs = {
...
test("deriveStage: static templates", () => {
  ...
  assert.equal(deriveStage({ type: "lifecycle_s2_implementing" }), "unknown"); // not a real event
...
test("isLifecycleEvent: rejects non-lifecycle types and malformed", () => {
  ...
  assert.equal(
    isLifecycleEvent({ type: "lifecycle_lane_open", ts: TS, repo: "x" }), // no linear_issue_id
    false,
  );
  assert.equal(
...
test("fold: last event wins for stage", () => {
  ...
  assert.equal(lane.worker, "w2"); // updated from s2_start
  assert.equal(lane.worktree, "feature/x"); // carried from lane_open
});
