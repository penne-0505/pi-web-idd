...
export function plannerBrief(rec: BacklogRecord, block?: { dec: number; inv: number }): string {
  ...
  return [
    ...
    "      <format>frontmatter に qa_schema: 5 / qa_status: planned / risk: Low|Medium|High|Critical。",
    ...
    "    DEC / AC / INV の見出しと質問の選択肢 label は UI の card にそのまま出るので、40 文字前後で 1 行に収まる長さにする。",
